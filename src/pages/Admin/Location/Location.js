      ...
      render: (text, location) => {
        return (
          <Fragment>
            <NavLink
              key={1}
              to={`/admin/location/edit/${location._id}`}
            ...
            <span
              onClick={() => {
                if (
                  window.confirm(
                    "Bạn có chắc muốn xóa vị trí " + location.name + " không?"
                  )
                ) {
            ...
            <NavLink
              key={3}
              to={`/admin/location/avatar/${location._id}`}
              style={{ color: "green", fontSize: 25, paddingRight: 10 }}
            >
              <FileImageOutlined />
            </NavLink>
          </Fragment>
